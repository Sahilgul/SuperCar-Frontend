...
const AppointmentConfirmationToolOutput = ({ data }: { data: { output: string | AppointmentData } }) => {
  // Parse the appointment data from various possible formats
  const parseAppointmentData = (): ParsedAppointment => {
    const defaultData: ParsedAppointment = {
      appointmentId: '',
      status: '',
      date: '',
      time: '',
    ...
    try {
      // Handle string input (could be JSON or other formats)
      if (typeof data.output === 'string') {
        // First try to parse as JSON
        try {
          const jsonData = JSON.parse(data.output) as AppointmentData;
          return parseStructuredData(jsonData);
        } catch (e) {
          // If not JSON, try other string formats
          return parseStringData(data.output);
        }
      } 
      // Handle object input
      else if (typeof data.output === 'object') {
        return parseStructuredData(data.output);
      }
    } catch (e) {
  ...
  // Parse structured data (JSON or object)
  const parseStructuredData = (data: AppointmentData): ParsedAppointment => {
    const result: ParsedAppointment = {
      appointmentId: data.appointment_id || '',
      status: data.status || '',
      date: data.details?.date || '',
      time: data.details?.time || '',
      carModel: data.details?.car_model || '',
      dealershipId: data.details?.dealership_id || '',
  ...
  // Parse string data (legacy formats)
  const parseStringData = (output: string): ParsedAppointment => {
    const result: ParsedAppointment = {
      appointmentId: '',
      status: '',
      date: '',
    ...
    if (codeBlockMatch) {
      ...
      while ((match = keyValueRegex.exec(dictContent)) !== null) {
        ...
        if (key === 'confirmation_id') result.appointmentId = value;
        else if (key === 'date') result.date = value;
        else if (key === 'time') result.time = value;
        else if (key === 'model') result.carModel = value;
      }
    } else {
      // Try other string formats
      const dateMatch = output.match(/(\d{1,2}(\/|\.)\d{1,2}(\/|\.)\d{2,4}|(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]* \d{1,2}(?:st|nd|rd|th)?, \d{4})(?: at )?(?: |, )?(\d{1,2}:\d{2}(?: AM| PM)?)/i);
      if (dateMatch) {
        result.date = dateMatch[1];
        result.time = dateMatch[4] || '';
      }
      
      const confirmMatch = output.match(/confirmation (?:id|number|code): ([A-Z0-9-]+)/i);
      if (confirmMatch) result.appointmentId = confirmMatch[1];
      
      const modelMatch = output.match(/(?:car|model|vehicle):\s*([^\n,\.]+)/i);
  ...
  const handleAddToCalendar = () => {
    try {
      const { date, time, carModel, appointmentId } = parseAppointmentData();
      ...
      const calendarContent = [
        ...
        `DTEND:${formatDateForIcal(endDate)}`,
        `SUMMARY:Test Drive - ${carModel || 'SuperCar'}`,
        `DESCRIPTION:Confirmation ID: ${appointmentId || 'N/A'}\\nYour test drive appointment`,
        'LOCATION:SuperCar Dealership',
        'STATUS:CONFIRMED',
        'SEQUENCE:0',
        'BEGIN:VALARM',
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `SuperCar_TestDrive_${appointmentId || 'Appointment'}.ics`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      URL.revokeObjectURL(url);
      ...
      <div className="p-4 bg-gradient-to-br from-blue-50 to-white">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm">
          <div className="space-y-3">
            {status && (
              <div className="flex items-center">
                <span className="font-semibold text-blue-800 w-28">Status:</span>
                <span className="capitalize">{status}</span>
                {status === 'confirmed' && (
                  <span className="ml-2 px-2 py-0.5 text-xs rounded-full bg-green-100 text-green-800">✓</span>
                )}
              </div>
            )}
            ...
            {dealershipId && (
              <div className="flex items-center">
                ...
                <div>
                  <p className="text-xs text-gray-500">Dealership</p>
                  <p className="font-medium text-gray-800">{dealershipId}</p>
                </div>
              </div>
            )}
            
            {typeof data.output === 'string' && !appointmentId && !dateTime && !carModel && (
              <div className="flex items-center">
                <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center mr-3">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-blue-700" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
