...
// const DealershipAddressToolOutput = ({ data }: { data: ToolOutputData }) => {
//   // Safely convert output to string
//   const outputString = typeof data.output === 'string' 
//     ? data.output 
...
//   // Extract store hours
//   let storeHours: string | null = null;
//   const hoursMatch = outputString.match(/(?:hours|open|hours of operation):\s*([^.]+)/i);
//   if (hoursMatch) storeHours = hoursMatch[1].trim();
  
//   // Extract phone number
//   let phoneNumber: string | null = null;
//   const phoneMatch = outputString.match(/(?:phone|call|tel|telephone):\s*([\d\s\(\)\-\+]+)/i);
//   if (phoneMatch) phoneNumber = phoneMatch[1].trim();
  
//   // Process address text
//   let addressText = outputString;
//   if (storeHours) {
//     addressText = addressText.replace(
//       new RegExp(`(?:hours|open|hours of operation):\\s*${escapeRegex(storeHours)}`, 'i'), 
//       ''
//     );
//   }
//   if (phoneNumber) {
//     addressText = addressText.replace(
//       new RegExp(`(?:phone|call|tel|telephone):\\s*${escapeRegex(phoneNumber)}`, 'i'), 
//       ''
//     );
//   }
...
const DealershipAddressToolOutput = ({ data }: { data: { output: string } }) => {
  ...
  try {
    ...
    addressText = jsonData.address || jsonData.output || '';
    phoneNumber = jsonData.phone || jsonData.phoneNumber || null;
    storeHours = jsonData.hours || jsonData.storeHours || null;
  } catch (e) {
    ...
    // Clean up address text if we extracted other info
    if (storeHours) addressText = addressText.replace(new RegExp(`(?:hours|open|hours of operation):\\s*${storeHours.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&')}`, 'i'), '');
    if (phoneNumber) addressText = addressText.replace(new RegExp(`(?:phone|call|tel|telephone):\\s*${phoneNumber.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&')}`, 'i'), '');
    addressText = addressText.trim().replace(/\s{2,}/g, ' ');
  }
