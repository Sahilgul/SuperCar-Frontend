...
{/* Message bubble with timestamp */}
        {
          message.content && (<div className={`mt-3 flex flex-col max-w-[85%] ${message.role === 'user' ? 'items-end' : 'items-start'}`}>
            <div
    ...
    <div className={`fixed bottom-4 right-4 transition-all duration-300 ease-in-out z-50 ${isMinimized ? 'w-16 h-16' : 'w-[420px] h-[600px]'}`}>
      ...
      {isMinimized ? (
        ...
        <div className="flex flex-col h-full rounded-2xl shadow-2xl overflow-hidden border border-gray-200 bg-gradient-to-b from-gray-50 to-white">
          ...
          {/* Quick response options */}
{chatHistory.length <= 1 && (
  <div className="bg-gradient-to-b from-blue-50 to-white px-3 py-2 border-t border-blue-100">
    ...
    <div className="grid grid-cols-2 gap-2">
      <div className="space-y-2">
        <button
          onClick={() => handleQuickResponse("What is the weather in Los Angeles?")}
          className="w-full px-2 py-1.5 bg-white border border-blue-200 text-blue-800 rounded-lg text-xs whitespace-nowrap hover:bg-blue-50 shadow-sm transition-all hover:shadow flex items-center justify-center"
        >
        ...
        <button
          onClick={() => handleQuickResponse("Get the address for dealership chi")}
          className="w-full px-2 py-1.5 bg-white border border-blue-200 text-blue-800 rounded-lg text-xs whitespace-nowrap hover:bg-blue-50 shadow-sm transition-all hover:shadow flex items-center justify-center"
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 mr-1 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          Dealership CHI Address
        </button>
      </div>
      <div className="space-y-2">
        <button
          onClick={() => handleQuickResponse("Check appointment availability for dealership_id 'nyc' on date 2025-04-01")}
          className="w-full px-2 py-1.5 bg-white border border-blue-200 text-blue-800 rounded-lg text-xs whitespace-nowrap hover:bg-blue-50 shadow-sm transition-all hover:shadow flex items-center justify-center"
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 mr-1 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <button
          onClick={() => handleQuickResponse("Schedule an appointment for user_id user123 at dealership_id 'nyc' on date 2025-04-01 at time '10:00' for car_model Tesla Model Y")}
          className="w-full px-2 py-1.5 bg-white border border-blue-200 text-blue-800 rounded-lg text-xs whitespace-nowrap hover:bg-blue-50 shadow-sm transition-all hover:shadow flex items-center justify-center"
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 mr-1 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
